...
public class TicketbearbeitungsService extends UnicastRemoteObject implements ITicketbearbeitungsService {
    ...
    @Override
    public List<String[]> getMitarbeiterTickets(int mitarbeiterID) throws RemoteException {
    List<String[]> tickets = new ArrayList<>();
    // Fetches tickets assigned to the employee that are currently "in Bearbeitung"
    String sql = "SELECT ticket.TicketID, ticket.BeschwerdeID, beschwerde.BeschwerdeID, ticketbearbeitung.TicketID, beschwerde.Beschreibung, ticket.Eröffnungszeitpunkt FROM ticket, beschwerde, ticketbearbeitung WHERE ticket.BeschwerdeID = beschwerde.BeschwerdeID AND ticketbearbeitung.TicketID = ticket.TicketID AND ticketbearbeitung.MitarbeiterID = ? AND ticket.Status = 'in Bearbeitung' AND ticketbearbeitung.Endzeitpunkt IS NULL";
    ...
    try (Connection con = DBConnector.getConnection();
        ...
        try (ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                ...
                ticketData[1] = rs.getString("Beschreibung");
                ticketData[2] = String.valueOf(rs.getObject("Eröffnungszeitpunkt"));
                tickets.add(ticketData);
            }
        }
    } catch (SQLException e) {
        e.printStackTrace();
        throw new RemoteException("Fehler beim Laden der Mitarbeiter-Tickets", e);
    }
    return tickets;
}
}